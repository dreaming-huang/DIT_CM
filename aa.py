def cm_training_losses_mutilStep(self, model,model_target,model_teacher, x_start, t,model_kwargs=None, noise=None,step=1):
        """
        ...
        with th.no_grad():
            with th.autocast("cuda"):
                ...
                # aa,_=ddim_res['pred_xstart'].chunk(2, dim=0) 
                # print(aa.shape,x_t.shape)
                # print(aa.equal(self._predict_xstart_from_eps(x_t,t,eps_tmp)))
                # print((aa-self._predict_xstart_from_eps(x_t,t,eps_tmp)).max())
                # print((x_tmp-x_t).max())
                # print(aa-self._predict_xstart_from_eps(x_t,t,eps_tmp))
                # print(x_tmp.equal(x_prev))
                # print((x_tmp-x_prev).max())
                # x_prev, _ = x_prev.chunk(2, dim=0)  # Remove null class samples
        ...
        # alphas_cumprod=th.tensor(self.alphas_cumprod,device=model_pred.device).sqrt()[t].view(model_pred.size(0), 1, 1, 1)
        # # weight= th.clamp(1/alphas_cumprod,1,100)
        # weight=1
        # # terms["loss"] = F.mse_loss(model_output.float(), model_target_output.float(), reduction="mean")
        # # terms["loss"] = th.mean(
